...
- **演奏**：屏幕键盘（**多指和弦**、滑奏、八度切换、**触屏按键力度**）、弯音/调制轮、
  Web MIDI（力度、CC1、CC64、CC123）。
- **MIDI 播放器**：内置 16 首曲目（古典与民乐为完整编配，现代主题为示范选段，另有琶音/音阶练习）、导入 `.mid` 文件、
  播放/暂停/停止/循环/变速/移调、现场录制、导出 `.mid` 与 **MP3**（lamejs 懒加载）。
- **钢琴卷帘编辑器**：网格画/拖/缩放音符、量化、移调、力度、撤销重做、试听与循环；
  底部键盘或外部 MIDI 键盘步进输入；保存为片段后可继续导出。
- **信号流视图 / 演奏模式**：节点化展示信号链，节点内实时波形/响应/包络可视化，
  按类型区分形状与颜色，可拖拽布局、旁通、移除/加回、点击展开完整参数；
  顶部性能条集成选曲、走带、录制与导出。
- **和弦识别**：同时按住多个音时监视器自动显示和弦名（含七/九和弦与转位）。
- **触屏交互**：鼠标/手指两套手势与提示；长按旋钮微调；按琴键位置控制力度；
  包络控制点大范围吸附拖动；可开关的振动反馈（支持振动马达的设备）。
- **响应式**：手机横竖屏、iPad 横竖屏、桌面分别优化——顶栏按设备重排（手机/平板竖屏
  两行、横屏单行），监视区在手机与平板默认收成带波形与频谱的紧凑条、桌面显示三块面板；
  信号流视图自动收起监视条（手机同时收起键盘），画布铺满整屏，平板竖屏 3 列、横屏 5 列两行。
- **主题**：深色 / 浅色 / 自动跟随系统（实时切换），另有高对比叠加模式；浅色为完整配色，含画布与监视器。
- **工作流**：67 个工厂预设（搜索/分类/本地保存）、随机音色、`.gs1.json` 导入导出、
  URL 分享、A/B 对比、撤销/重做（Ctrl/Cmd+Z）、离线渲染导出 MP3。
- **内置指南**：随应用离线的双语手册——合成器理论（物理、谐波/傅里叶、采样与混叠、滤波器理论）、
  **合成器简史**（Telharmonium 到 Web Audio）、**合成器分类**（11 种发声方法 / 架构 / 演奏方式）、
  **模块原理详解**（本机 polyBLEP、Moog 梯形模型、SVF、包络、LFO、矩阵、效果链、数字实现）、
  新手教学、使用帮助、播放器/信号流说明与「从零捏一个音色」分步教程。
- **双语**：中文 / English 一键切换（预设库底部），语言随布局一起持久化；
  AMP/FILTER ENV 数值可直接输入或滚轮/方向键调节。
- **布局**：模块可折叠 / 拖拽排序、信号流节点拖拽、悬浮可隐藏键盘、高对比主题，全部本地持久化。
