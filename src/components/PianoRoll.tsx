...
/**
 * The grid's viewport origin, in client coordinates.
 *
 * A gesture has to be measured against *one* origin for its whole life.
 * Selecting notes renders the selection bar and the inspector, which push the
 * grid down (measured on WebKit: 90 px). A marquee whose start was measured
 * before that shift and whose end is measured after it covers a pitch range
 * neither end asked for: the box clears the selection, the bar disappears, the
 * grid moves back, and the two ends flip between coordinate systems as the
 * pointer moves -- so a box dragged over the notes selects nothing.
 */
type GridAnchor = { left: number; top: number };
...
type Gesture =
  | {
      ...
      beat: number;
      pitch: number;
      /** The grid origin `beat`/`pitch` were measured against, reused by every
       *  move of the same gesture (see GridAnchor). */
      anchor: GridAnchor;
    }
  | {
      kind: 'note';
...
export function PianoRoll({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  /** The origin the marquee in flight was measured against (see GridAnchor). */
  const marqueeAnchor = useRef<GridAnchor | null>(null);
  ...
  const gridBox = (): GridAnchor => {
    const el = gridRef.current;
    if (!el) return { left: 0, top: 0 };
    const rect = el.getBoundingClientRect();
    return { left: rect.left, top: rect.top };
  };
  /** `anchor` defaults to the live box, so every caller that is not inside a
   *  gesture keeps today's behaviour; a gesture passes the origin it captured
   *  at pointerdown, because the gesture itself can move the grid. */
  const beatAt = (clientX: number, anchor: GridAnchor = gridBox()): number =>
    Math.max(0, (clientX - anchor.left) / zoomRef.current);
  const pitchAt = (clientY: number, anchor: GridAnchor = gridBox()): number => {
    const row = Math.floor((clientY - anchor.top) / ROW_H);
  ...
  const onGridPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const anchor = gridBox();
    gesture.current = {
      kind: 'tap',
      x: event.clientX,
      y: event.clientY,
      beat: beatAt(event.clientX, anchor),
      pitch: pitchAt(event.clientY, anchor),
      anchor,
    };
  };
  ...
  const onGridPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (rect) {
      const anchor = marqueeAnchor.current ?? gridBox();
      const grown: GridRect = {
        beat0: rect.beat0,
        beat1: beatAt(event.clientX, anchor),
        pitch0: rect.pitch0,
        pitch1: pitchAt(event.clientY, anchor),
      };
    ...
    if (g.kind === 'tap') {
      ...
      const box: GridRect = {
        beat0: g.beat,
        beat1: beatAt(event.clientX, g.anchor),
        pitch0: g.pitch,
        pitch1: pitchAt(event.clientY, g.anchor),
      };
      gesture.current = null;
      marqueeAnchor.current = g.anchor;
