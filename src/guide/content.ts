  ...
  // ================================================================ modules
  {
    ...
    blocks: [
      ...
      { kind: 'h', text: ['效果 FX', 'Effects'] },
      {
        ...
        items: [
          [
            '**混响**：自研 Freeverb 结构（每声道 8 个阻尼梳状滤波器 + 4 个全通扩散器，带立体声扩展）。`SIZE` 控制衰减长度、`DAMP` 控制尾音高频衰减、`WIDTH` 控制立体声宽度、`PRE` 是 0–100 ms 预延迟、`MIX` 是干湿比。延时线刻意保持静止：反馈环路里的移动抽头会在延时变短时注入能量，让尾音越积越大。',
          ...
          [
            '**效果路由图**：想更自由就打开它（FX 模块标题旁、信号流工具栏 ⌗、或设置抽屉）。**拖卡片标题**可以摆放位置，连线会跟着走（摆放属于工作区，换音色/重开都在，「重置布局」恢复默认）。同一个效果可以用在多个节点，**每个节点一套状态**：延迟池有两条 2 秒线、卷积池有两个节点（IR 分区谱共享，尾音各自独立），编辑器提示行显示延迟池还剩多少可分配时长，超过池容量的同类节点会被禁用并说明原因。节点之间可以连线：每个节点最多两路输入（取干声或任一更靠前节点的输出，各自带增益），任一节点都能送到输出——串联、并联、一分多路、两路求和都行。拖线，或先点输出再点输入；点连线断开。手机默认列表视图。没动过它时声音仍由信号链决定；第一次连线会以当前信号链为起点，所以不会突然变声，「从信号链重建」可随时还原。',
            '**Routing graph**: for more freedom, open it from the FX module, the signal-flow toolbar (⌗) or the settings drawer. Drag a card by its title to arrange the board — the wires follow, the arrangement belongs to the workspace (it survives patch changes and reloads) and “Reset layout” restores the default. The same effect can appear in several nodes, each with its own state: the delay pool holds two 2-second lines and the convolution pool two nodes (the partition spectra are shared, the tails are not). The hint line shows how much delay time is still available, and a node past the pool is disabled with the reason. Nodes can be wired: each sums up to two inputs (the dry signal or any earlier node, each with a gain) and any node can reach the output — series, parallel, fan-out and summing. Drag a wire, or tap an output then an input; click a wire to cut it. Phones default to the list editor. Until you change it the chain still decides the sound, and the first edit starts from that chain, so nothing jumps; “Rebuild from chain” restores it.',
          ],
          [
