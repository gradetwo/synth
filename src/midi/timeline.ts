...
export function layoutNotes(notes: MidiNote[], duration: number): Block[] {
  const span = duration > 0 ? duration : 1;
  return notes.map((note) => {
    const start = Math.max(0, Math.min(1, note.start / span));
    const end = Math.max(start, Math.min(1, (note.start + note.duration) / span));
