/**
 * The complete pre-split release history (P141), frozen as a test fixture.
 *
 * This file is generated data, not shipped code: it exists so
 * `changelog.test.ts` can prove that `CHANGELOG` plus `CHANGELOG_ARCHIVE`
 * is still the list the app shipped before the 30-entry cap, even if git
 * history is rewritten or the test runs from a tarball. Same shape as the
 * pre-split `src/changelog.ts` array, head entry excluded.
 */

import type { Release } from './changelog';

export const CHANGELOG_HISTORY: Release[] = [
  {
    "version": "2.0.7",
    "date": "2026-09-14",
    "kind": "fix",
