...
// is a **mipmap of short tables read with linear interpolation** — so neither
// was covered by the BH-7 scans above. This section is the P9.5 review of those
// two sources above 1 kHz, measured with the same P9.1a ruler as the rest of
// the file (four whole seconds, exact bins, +-8 bins around every harmonic,
// clean modulation matrix, real wasm, every scenario's whole parameter block
// pinned by `renderFloor`).
//
// The review's finding is that the batch's acceptance line -- ">= 1 kHz
// wavetable and sampler sources stay under -60 dB of non-harmonic energy" -- is
// **not met**, and cannot be met without changing the sound:
//
//   * every factory bank measures between -25.8 and -46.7 dB, and the sampler
//     between -29.9 and -31.0 dB. The worst case is the vocal bank at C8
//     (4186 Hz), -25.8 dB.
//   * the *levels* are band-limited -- `dsp/wavetable.rs` has a test per level
//     and `dsp/sampler.rs` a stopband test -- so what is left is the
//     **interpolator**. A 16- or 32-sample level read with linear interpolation
//     is a piecewise-linear approximation whose own error is a train of high
//     harmonics. The ones above Nyquist fold back to `SR - k*f0`, which is not
//     on the note's grid: at C7 the strongest lines sit on exactly
//     48000 - 21*2093 = 4047 Hz, 48000 - 20*2093 = 6140 Hz, 48000 - 19*2093 =
//     8233 Hz, and so on (measured on this build; an independent JS replica of
//     the read reproduces the same lines to the bin).
//   * the 1/N^2 signature of linear interpolation was measured directly on the
//     same seven-harmonic content: -24.4 dB at 16 samples, -34.0 at 32, -46.4
//     at 64, -58.5 at 128, -82.6 at 512, -106.6 at 2048. Nearest-neighbour is
//     -10.4 dB, and a 4-point cubic only buys 6 dB over linear (-30.4), so this
//     is not a rounding detail.
//   * a second, independent measurement -- a rectangular window over exactly
//     8372 cycles of C7, where every harmonic is on a DFT bin and Goertzel
//     reads the lines with no leakage -- agrees within 0.2 dB (-30.1 / -37.5 /
//     -32.7 dB for the vocal/organ/glass banks).
//
// So this section pins the floor where the engine actually is, with a ceiling
// about five dB looser than the measured worst case per source (the same kind
// of headroom the P9.1b bounds above use, and like them nowhere near -60). It is
// deliberately **not** the -60 dB line: asserting -60 here would fail the gate,
// and quietly writing -30 without saying why would hide the finding. The
// numbers, the attribution, the spectral evidence and the follow-up batch are
// written up in `docs/notes/band-limited-oscillators.md` (the "P9.5" section).
// Fixing it means reading a longer level or a better interpolator, i.e. changing
// the timbre and every preset fingerprint, so it belongs to a batch of its own
// rather than to this review.
{
  ...
  // whole factory content. Each bound is a *ceiling* about five dB looser than
  // the measured worst case (the same headroom the P9.1b bounds above use): it
  // fails on a regression, and it is nowhere near the -60 dB line the plan
  // asked for, because the engine is nowhere near it either.
  ex.gs_wavetable_clear();
  for (const [name, pw, bound] of [
    ...
    check(
      ...
      `${at([84, 96, 105, 108], values)} (worst ${worst.toFixed(1)} dB, ceiling ${bound}, measured +5 dB headroom; the -60 dB line is NOT met — see the section note)`,
    );
  }
  ...
  check('the review cycle imports', importCycle(sawCycle) === 0, 'code 0');
  {
    ...
    check(
      ...
      values.every((v) => Number.isFinite(v) && v < -26),
      `${at([84, 96, 105, 108], values)} (worst ${worst.toFixed(1)} dB, ceiling -26; the -60 dB line is NOT met — see the section note)`,
    );
  }
  ex.gs_wavetable_clear();
  ...
  // levels and a fractional rate in every case).
  const sampleNotes = [84, 96, 108];
  {
    ...
    check(
      ...
      values.every((v) => Number.isFinite(v) && v < -25),
      `${at(sampleNotes, values)} (worst ${worst.toFixed(1)} dB, ceiling -25; the -60 dB line is NOT met — see the section note)`,
    );
  }
