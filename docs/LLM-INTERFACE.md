...
| **P13.2 只读 + 渲染 + 测量** | `mcp/server.mjs` + `gs1.describe`/`params.list`/`presets.list`/`patch.get`/`render`/`analyze`/`gate` + `npm run mcp` | 工具级单测（schema、参数校验、拒绝路径）+ 一次**黄金会话**：脚本化调用同一串工具两次，结果逐字节相同；手写 JSON-RPC 的 `initialize`/`tools/list`/`tools/call` 有测试；**无新增运行时依赖**（`package.json` 的 `dependencies` 仍为空） |
