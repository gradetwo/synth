...
- **P6.2b（进行中，v1.85.0 完成第一步）**：
  ...
  - ⬜ **第二步：剩下一对 ±23.4 Hz 乘性边带**（−92.5 dB、左右对称、与音量无关）——是 AM 而不是噪声，嫌疑最大的是**限幅器的慢增益环**。要先用 `limit_gain` 的时间序列（或临时改检波时间常数）确认，然后让限幅器在拐点以下**增益严格为 1 且不更新**。
  - ⬜ **第三步**：底噪压到 −120 dB 以下后，用同步参考法分开量「2× 上采样 + 95 抽头抽取滤波器」与「重启点亚采样对齐」的贡献，达标后把 `混叠 ≤ −60 dB` 写进门禁（P6.2 才算完整）。
