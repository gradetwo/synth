...
> **本计划（v1.79.0–v1.97.0，14 个批次）已全部交付。** 下一阶段的开发/完善/改进计划见
> [`docs/NEXT-PLAN-2.md`](NEXT-PLAN-2.md)（P9 音频质量二期 / P10 创作与工作流二期 / P11 工程质量二期 /
> P12 产品化与生态，共 21 个批次，含验收、门禁、顺序与风险）。
...
- **P6.2b（进行中，v1.85.0 完成第一步）**：
  ...
  - ✅ **第三步：专用带限同步振荡器（v1.97.0 完成）**——`gs_daisy.cpp` 里新增 minBLEP/minBLAMP 核（启动时由 Blackman 窗化 sinc 低通生成，static BSS，无动态全局构造），`gs_voice_osc_sync_block` 改为**专用朴素波形振荡器**，在同一 double 相位上同时补「自身回绕」与「主振重启」的 BLEP/BLAMP，并用 68 槽环形累加器把朴素信号延迟 32 个过采样样本让对称核因果落地（主振仍走 DaisySP，相位由桥接层 double 跟踪）；保留 2× + 95 抽头抽取。实测（主振 220 Hz、从振 1.41×、稳定后整秒/矩形窗/精确 bin）：host 锯齿/方波/三角 **−72.3 / −71.1 / −75.7 dB**，真 wasm 门禁 **−68.7 / −68.6 / −77.3 dB**（改动前 −32.1 / −33.5 / −49.5），未同步对照均 −0.0 dB；时域 wasm 相关 0.9995–0.9997、峰值 0.117–0.136、最大步进 0.093–0.129。门禁与 Rust 单测都改为**断言 ≤ −60 dB**，「刻意省略」的注释已删除；`docs/notes/hard-sync-aliasing.md` 记录第三步；`docs/USER-GUIDE.md` 同步更新。
  - **已知边界**（未放宽门禁）：门禁与单测按 1.41× 断言；其它非整数比仍会退化（锯齿 x1.7 −49.8、x7.1 −31.2、x11.3 −35.2，方波 x4.3 −37.8），但新实现一律优于旧代码（旧值 −22…−34）；这些残差对 BLEP 核参数（N=32/48、fc=0.15/0.22）不敏感，且与引擎自身同音高普通锯齿的底噪（1562 Hz 时 −35.1）同量级，判为**更上层的引擎/波形底噪**而非重启修正本身。窄脉冲 pw=0.05 在 1.41× 为 −36 dB（pw=0.15 为 −61.7），门禁把方波 pw 固定 0.5。若要覆盖这些，需要另开一批查上层底噪来源。
  - （历史）⏸ **第二步之后的搁置记录**：两次尝试都失败并已回退（第一次用 DaisySP 的平滑值估台阶：−32 → −1.6 dB；第二次按建议改用**裸波形**估台阶：锯齿 −1.9 dB、三角 −1.9 dB、方波 −33.5 dB 等于没改善）。结论：**不要在 DaisySP 的输出上打补丁**——方波的两次跳变各自已被它的 polyBLEP 处理，再减一次裸台阶等于把带限好的信号切开；锯齿在重启点附近的平滑本身就是它带限的一半。正确形态是**给同步做一个专用带限振荡器**：直接输出朴素波形，在同一采样位置同时做「回绕的 BLEP」与「重启的 BLEP/BLAMP 残差」（位置用已验证正确的小数 `t`，幅度用同一套朴素波形差值），共享同一份相位判断。做完用稳定度量（整秒 + 矩形窗 + 精确 bin）复量，达标后把 `混叠 ≤ −60 dB` 写进门禁。两次尝试的实测数字与调试输出都在 `docs/notes/hard-sync-aliasing.md`。
...
| 6b | P6.2b 混叠度量与非周期底噪 ✅ 度量可用；**专用带限同步振荡器 ✅ v1.97.0（−68.7 dB）** | P6.2 | 中 |
