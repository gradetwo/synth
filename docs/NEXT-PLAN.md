...
- **P6.3b 双滤波串联/并联 + 混合量** ⏳ 待做（下一批；任务书见 `.tmp/brief-p63b.md`）
  - 要点：第二个滤波级（类型/截止/共振/驱动独立）、串联/并联接法、并联混合量；默认关闭时旧音色逐位不变。
  - 验收：频域（串联 = 两级幅频之积、并联 = 加权和，均对解析式；斜率变陡）；时域（切换接法与扫动无爆音）；arena 与基准数字前后对比。
...
| 9 | P6.3 滤波补全：P6.3a SEM 连续多模 ✅ v1.89.0；P6.3b 双滤波 ⏳ | — | 中 |
