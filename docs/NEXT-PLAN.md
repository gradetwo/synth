...
| **P2 完整版：效果路由图（前馈 DAG）** | ✅ v1.72.0 / v1.73.0 | 引擎（6 节点 + 干声源 + 输出汇、每节点 2 路输入带增益、扇出、无环）+ 编辑器（画布拖线/点按连线/列表视图、三处入口、随音色保存）。**仍不等同于任意拓扑**：连线只能从左向右（保证无环、渲染序固定），没有独立的效果器实例与自由布局画布 |
...
> 本轮把 A（节点图引擎 + 编辑器 + 兼容）、B（编曲随分享码）、C（工程项收尾）三项做完，
> 状态如下；再往下的候选见本节末尾「下一步候选」。
...
**A3 兼容与文档（可与 A2 合并）— ✅ 已完成（随 v1.73.0 / v1.75.0）**
1. ✅ 分享码/预设无需升 schema：图参数是普通参数（append-only id），旧码读入即默认「图关闭」，
   新码在旧版本里多出的值被忽略；`share.test.ts` 补了「图随码走 + 旧码不打开图」的用例。
2. 文档：USER-GUIDE 新增「信号图」章节、应用内指南、ROADMAP 收尾；`docs/notes/` 记录内存/预算实测。

### B. 曲目与层混音随分享码 — ✅ 已完成（v1.74.0）
...
1. **节点图续** ✅ 基本完成：~~自由布局画布~~ v1.76.0、~~边增益可视化编辑~~ v1.77.0、
   ~~端口与连线的键盘/读屏~~ v1.77.0、~~效果节点多实例~~ **v1.78.0**（算法混响与四个插入效果每节点一套状态；
   延迟线与卷积因内存上限仍单实例，第二个同类节点直通且编辑器禁用该选项）。
   剩下的是打磨性质的工作：更细的焦点顺序、读屏朗读文本、以及「把延迟/卷积也做成多实例」——后者需要
   改内存模型（延迟线 768 KB、卷积 787 KB/份，8 MiB arena 里放不下六份），属于引擎级重构，单独立项。
...
3. ~~**曲目随分享码的压缩**~~ ✅ 已完成（v1.77.0）：`CompressionStream('deflate-raw')` 先压缩再 base64，
   五分钟曲目 36,390 → 1,516 字符（96% 更小）；`gs1.2.` 前缀区分压缩码，旧码与 `.gs1song` 不变。
4. **`songs.ts` 懒加载** — ❌ **本期不做**（收益低、成本中、动核心）：内置曲库数据约 30 KB 源码（≈4 KB gzip）
   仍在初始 chunk，但它必须在启动时就有——曲库构造时要加载「当前曲目」，而 `Track.song` 是同步字段；
   拆出去要把 `Track.song` 变成异步获取，播放器 / 曲库 / 钢琴卷帘 / 导出 / 分享码全都要配合，换来 4 KB。
   已记在案，等哪天真需要再动（例如把内置曲目整体改成按需下载的曲包）。
5. **P3 无障碍（按约定不做）**：若改变约定，从画布连线、屏幕阅读器标签、键盘全流程开始。
