...
| 门禁 | `npm run verify`：clippy（correctness/suspicious/perf）+ Rust **152** + Vitest **287** + lint + build + wasm（ABI/无全局构造/零分配）+ dist + 体积（总 1684.3/1700 KB、初始 JS 153.2/165 KB、CSS 19.0/22 KB、WASM 56.8/230 KB）+ 时域&频域音质 + 负载基准 + DSP 指纹 |
| E2E | Chromium **103** 项（本地判据）；WebKit/Firefox 由本地 `npm run nightly` 与 CI 的 `nightly` 作业（`on.schedule`）跑 |
