...
6. **同一键上新的 `noteOn` 会作废更早排队的 `noteOff`**。重触发是演奏里最普通的事：旧的释放事件若留
   在队列里，就会在新音符之后触发，而 `gs_note_off(note)` 释放的是**此刻在该键上发声的那个声部**——也就是
   新音符。听感是一按下去就被切掉（或只剩碎片），在"反复点轨道预览按钮"这一类操作上最容易出现。规则按键盘
   的语义来：键已经松开又按下，只有新的这次还欠一个释放；新事件自带自己的 `noteOff`，正常入队。
7. **`cents` 可选，随音符一起下发并在该音符自己的帧上生效**。它曾经是一条独立的 `tuning` 消息、到达即写：
   宿主给未来的帧排音时，那条消息会去改**正在响的**同一个键，于是"预排"的音符会把当前声音弯走——同一个
   仓库里一个浏览器听得出、另一个听不出，就是这种时序差。微分音属于它自己的那个音符，所以现在走同一条事件。
8. **`noteAt` 与 `noteOnAt` 是完全的别名**（同一 `case` 分支、同一套校验）。保留两个名字是为了
   主机侧可读性，不是两种语义。
