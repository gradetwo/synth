...
`--core` 只跑核心子集（8 个 spec：iPhone/iPad 视口、触屏、排版、启动、路由图、分享、抽屉、主题），`--all`
跑全量。文件清单是 `scripts/nightly-e2e.mjs` 里的常量，本文件不复述，免得两处各说各话。

**本机的 systemd 定时器跑 `--core`**（2026-09-15/16 慢轨实测后定的口径）。默认子集在本机跑不完：
Firefox 默认子集（核心 + 视觉 + 音频，18 个 spec）**14.8 min** 通过（68 passed / 4 skipped——那 4 条 skip
是宿主没有音频后端，不是绿），而 WebKit 默认子集 **>96 min 仍未完成**、WebKit `--all` **>132 min 仍未完成**；
