...
> **可移植经验**：这些坑里与具体产品无关的部分（输入合成、动画时钟、视口通知、媒体特性仿真、
> 引擎/平台矩阵、以及「怎么判断是环境还是产品」的方法论）已抽象成一份独立清单：
> [`webkit-testing-pitfalls.md`](webkit-testing-pitfalls.md)。其它项目可以直接拿去用。
...
- **定时**：`scripts/install-nightly.sh` 安装 `scripts/systemd/gs1-nightly.{service,timer}`（用户定时器，
  每天 03:00，`Persistent=true`，`TimeoutStartSec=5h`）；本机当前 session 没有 user bus，安装脚本会给出提示
  （`systemctl --user` 需在登录会话里跑）。unit 跑的是 `--engines=webkit,firefox --core --update`，所以两个内核都会进记录。
  **2026-09-15/16 慢轨实测后口径从默认子集改成 `--core`、超时 3h → 5h**：Firefox 默认子集 14.8 min 通过
  （4 条 skip = 宿主无音频后端，不是绿），WebKit 默认子集 >96 min 未完成、WebKit `--all` >132 min 未完成
  （WebKit `--core` 历史 22.7 min，本轮 >28 min 未完成）⇒ 串行下界 >111 min 而 WebKit 无上限，3h（180 min）
  可能被超过，超时后没有结果行。`--all` 仍是 CI 的 schedule `nightly` 作业（两引擎各一遍），所以 timer 走 `--core` 不是丢覆盖。
  两条都由 `scripts/verify-ci.mjs` 断言（CI 必须两遍 `--all`；timer 必须 `--core`、不得 `--all`）。
...
- ~~`export.spec.ts` 的 MP3 离线渲染在 Firefox 下超出用例时限（渲染比 Chromium 慢很多）。~~
  **更正（本轮实测修复，工作树未提交）：不是「慢」，是 Gecko 上离线导出必然抛异常。** Firefox
  没有 `OfflineAudioContext.suspend`（MDN BCD `api.OfflineAudioContext.suspend` → firefox
  `version_added: false`，bug 1265406；`resume` 只实现了部分且总是 reject），而
  `src/audio/render.ts` 原来用 `ctx.suspend(t)` 排音符，于是 **MP3 与 WAV 导出都在渲染前就**
  `TypeError: ctx.suspend is not a function`：页面显示 `MP3 导出失败：o.suspend is not a function`，
  E2E 表现为等下载等到 200 s 超时。FF155/Linux 探针：`OfflineAudioContext.prototype` 自有键
  `[startRendering, length, oncomplete, constructor]`，`typeof proto.suspend === "undefined"`。
  原始证据在 `.tmp/triage/ff-probe-offline2.log` / `ff-export-run1.log` / `evidence.md`。
  **修法**：`render.ts` 把整首歌按绝对帧经 `processorOptions.notes`（与 `routes` 同一条通道）交给
  worklet；worklet 用与 `noteAt`/`noteOffAt` **同一个校验器**（`timedEventFrom`）预填
  `scheduledNotes`，再在精确帧上逐个应用。只有在 `typeof ctx.suspend === 'function'` 的引擎上才
  保留原来的 suspend 排程，其余（Gecko）走按帧路径。
  **两条排程不是逐位相同（实测，Chromium，44.1 kHz，默认补丁）**：`suspend` 在**下一个 128 帧渲染
  量子边界**恢复渲染，所以音符从「起点所在的量子边界」起音；按帧路径从**精确帧**起音（两者随后都有
  同样的一个量子声部启动延迟）。实测 0.5 s 处的音符（帧 22050）首次可闻：suspend = **22288**，
  按帧 = **22194**；差 `(128 - atFrame % 128) % 128` 帧（0–127 帧，44.1 kHz 下 0–2.88 ms），按帧
  路径只会更早、不会更晚。另一个 0.1 s 起的音符（帧 4410）实测 4624 → 4554（差 70 帧，符合公式）。
  因此**能 suspend 的引擎保留旧排程**（导出逐位不变），按帧路径只作为没有 suspend 的引擎的回退。
  验证：Chromium 上改造前后**逐样本 SHA-256 相同**（单音 `2bda4a65…`、12 音带 pan 乐句
  `4af79d0c…`）；把 `OfflineAudioContext.prototype.suspend` 删掉强制走回退分支后哈希变为
  `7282bc3d…` / `340f4106…`（正是上面那个量子量化差）。`e2e/export.spec.ts` 在 firefox 上
  `--workers=1 --retries=0` **2 passed (1.5m)**（修复前同一条 MP3 用例 4.1m 失败）。
  注意：**WAV 导出没有浏览器用例**（`e2e/export.spec.ts` 只有 MP3 与 MIDI 两条；WAV 按钮在
  `src/components/PlayerPanel.tsx`），它走的是同一个 `renderPatchToBuffer`，所以一起被修好，但
  「修好」本身没有 E2E 断言。
