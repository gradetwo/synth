...
| 客户端状态 | 回滚后的实际情况 |
| :--- | :--- |
| 新访客 / 没有 SW / 硬刷新 | **立刻是 vX**。SW 的导航请求是 `fetch('./?v=<cache>', { cache: 'no-store' })`，从不复用旧 `index.html`。 |
| 已装 PWA，正在跑 vW | **不会自动退回**。它要等 `registration.update()`（切回标签页、或最长 30 分钟一次）发现 `sw.js` 变了 → 装成 waiting → 横幅出现 → 用户点「立即更新」→ 激活 vX 并 reload 一次。 |
| 离线中的 PWA | 一直跑它自己那份（vW），直到联网并走完上面的流程。服务器上做什么都够不着它。 |

**矛盾点（本批要求写清的那一条）**：第二行里横幅会显示「新版本已就绪 · **vW** · …」——它报的是
**用户自己正在跑的 vW**，也就是**我们刚刚回滚掉的那个版本**；而点下去真正装上的是 vX。也就是说：

> 回滚后，**横幅的标签指向被回滚的版本**，但**按钮的结果**是回滚目标。

为什么会这样：横幅要报 waiting worker 的版本，SW 必须先把这个版本号告诉页面（现在的 `sw.js` 里没有版本号），
也就是要动 SW 的 message 协议 + 横幅文案 + i18n，还会吃首屏 JS 预算。那是一件独立的小批，
不属于回滚工具本身——本批**只把它写清、并用脚本断言「横幅一定会重新出现」这一半**：

- 断言（`rollback.mjs` 第 7 步）：回滚后的线上 `sw.js` 必须与快照逐字节一致、cache 名相同。
  只要它与回滚前**不同**，已装客户端就一定会拿到 waiting worker、横幅一定会重新出现；
  如果它和回滚前一模一样，说明这次「回滚」跟线上是同一份字节，脚本会直接报红。
- 未断言（已知限制）：横幅上写的版本号是否正确。这需要上面那条 SW 协议改造。

**操作含义（回滚后照做）**：

1. 回滚后**先不要**把 vW 再发一次，除非确认已装客户端已经更新到 vX；否则会出现「发上去又被横幅推回去」的来回。
2. 主动告知用户「刷新一次即可」：横幅是到达已装客户端的**唯一**通道，而且它不会自己消失。
3. 「回滚成功」只对**线上新访客**是可证的事实。已装客户端的退回无法被服务器验证，
   脚本也不假装能验证——这正是它把这段说明每次都打出来的原因。
