...
**P12.3 分享与协作**（1 批）：分享码携带图模板/工程、链接预览卡片、短链与二维码；尺寸上限与 `.gs1song` 降级；往返一致性单测。
**P12.4 分发与回滚**（1 批）：版本化 CDN 保留 N 个版本、回滚开关与演练脚本、更新提示联动；线上核对纳入演练。
**P12.5 无障碍（需明确变更最初的「不做 a11y」约定）**（1–2 批）：全键盘流程、ARIA live 播报、Lighthouse a11y ≥95。
...
| 15 | P12.1–P12.4 产品化 | P10/P11 | 中大 | v1.112.0–v2.0.0 |
| 16 | P12.5 无障碍（需同意） | — | 中大 | 待定 |
