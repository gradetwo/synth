# 下一阶段开发 · 完善 · 改进计划（v1.111.0 起 · P9.7 已立项）
...
15. **bench 的「超预算块占比」在本机不可复现（v1.111.0 发布时发现）**：同一份代码、同一天、几乎相同的机器探针，两次结论相反——08:0x 的发布跑 `p50 1117 µs`、`cpu probe 931/1600`、`load 4.0` ⇒ **4 条 timing 全部 judged 且 PASS**；10:15 的 verify 跑 `p50 1354 µs`、`cpu probe 933/1600`、`load 3.7` ⇒ **`66/2250 blocks over 2667 µs after removing host stalls`** 判红。也就是说**同代码 0 vs 66 个超预算块**，而 `cpuProbe()` 看不出差别（两次都 ~930 µs）⇒ 这条判定当前测的是环境。**注意**：`src/fuzz.test.ts` 的「10000 inputs < 4000 ms」也在负载高时反复假红（实测 4044 / 5267 ms），属同一类。**要做**（单独小批次）：让这两处与 `bench` 的 IR 检查用**同一套** host-busy 判据（现在同一跑里 IR 判 skipped 而普通负载判 red，自相矛盾），或改用不随主机漂移的判据（例如相对基线/多窗中位数的稳定性），**而不是**调阈值或加长预算。在那之前：**发布时必须在安静主机上单独跑**，并且绝不能把 `| tail` 之类的管道当成退出码来源。
...
- **用户已拍板（2026-09-14）**：**(1) 与 (2) 都授权**，接受由此产生的音色变化与全部 81 条指纹重录。判据只有一个：150 个全新场景的**坏点数压到 0/150** 且不牺牲 P9.1b 的带限余量。重录必须走 `presets:update(--2x) -- --reason "<具体原因>"`；若 `test:dsp` 也变，必须先能解释变化再显式重录（说不出原因就不许重录）。
- **验收**：C7 + 工厂滤波下 150 个全新场景的坏点数显著下降（目标 0/150），且**不改动** P9.1b 刚挣到的带限余量；门禁补一条**确定性**（非抽签）的断言。

**P9.7 波表/采样高音区带限修到接近 −60 dB**（新增，P9.5 的修法批次；用户 2026-09-14 授权改音色 + 重录全部指纹）
- **输入**：P9.5 的实测与归因（见上）。要修的是**线性插值读短 mip level**（表长 16→2048 点 = −24.4→−106.6 dB，1/N²），以及采样 `state.position` 的 **f32 精度**边带（以 loop 重复率为间隔；f64 复刻即消失）。
- **首选**：按 1/N² 律**加长 level**（128 点 ≈ −58.5、256 点 ≈ −70.6）；插值器升阶（4 点三次仅 +6 dB）单独不够，可并用。
- **必须报账**：≥1 kHz 最差 dB、**内存增量**（arena 8 MiB 零分配必须保持 `gs_alloc_violations()==0`）、加载/构建时间、是否触碰 ABI/参数；**体积若上涨必须停下来报我**，不要自行 rebase 阈值。
- **门禁**：把 `verify-audio.mjs` 的 P9.5 顶棚从「修复前实测 +5 dB」收紧成「**修复后实测 + 余量**」，保留两把独立尺子（BH-7 + 矩形窗 Goertzel）与确定性、真 wasm、时域 Bernstein 上限。
- **相容性**：**改音色 + 重录全部 81 条指纹**（用户已授权），走 `--reason` 协议；`test:dsp` 若变必须先能解释再重录。
- **验收**：逐音高 × 音源都推进到**接近 −60 dB**；P9.1b 的带限余量不回退；自证（部分回退修复 ⇒ 门禁红）。
...
| 11c | **P9.7 波表/采样插值器**（把 ≥1 kHz 的非谐波能量从 −25.8…−46.7 dB 修到接近 −60；**改音色 + 重录全部指纹，用户已授权**） | P9.5 | 中 | v1.113.0 |
