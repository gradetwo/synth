...
1. **引擎非谐波底噪**：普通锯齿在 1562 Hz 的非谐波能量 **−35.1 dB**；硬同步残差随比值退化（锯齿 x7.1 −31.2、方波 x4.3 −37.8），与该底噪同量级 → 同步修正已不是瓶颈。**正在调查**（`.tmp/brief-noise-floor.md`；**轨道 `pnoise` 在跑**：今天的数字重测 + 消融 + 根因，见 `.tmp/brief-pnoise.md`）。
