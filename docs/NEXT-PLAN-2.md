...
30. **加内容的批次必须跑既有的「全内容守卫」（慢轨第二次扫描抓到，v2.0.4 后登记）**：P12.2 的 10 条新预设里，`crushbass`（低八度 + 600 Hz 滤波 + 4bit/12× 粉碎 + `PATCH_GAIN 0.442`）在 **`src/audio/worklet-processor.test.ts` 的既有守卫**（对**每条**工厂预设弹 middle C、断言 **peak ≥ 0.003**）下读到 **peak 0.0020（≈ −54 dBFS）** ⇒ **用户几乎听不见**。P12.2 的快轨只跑了自己**新写**的门禁（`preset-audition`，门槛 RMS > 1e-4，比既有守卫松两个数量级），所以放过了它——**是慢轨抓到的**。**修法**：把该预设修到守卫绿并保持「位粉碎低音」的音色，指纹按协议重录（只动这一条）；**并把 `preset-audition` 的门槛对齐既有守卫的口径**。**规矩（已写进 `/.tmp/discipline.md` 第九节）**：①加内容 ⇒ 快轨必须跑 `worklet-processor` 的全预设扫描 + `preset-loudness` + `song-quality`；②新门禁的阈值不许比既有守卫松；③宣布完成前先问「仓库里还有什么门禁会看到我这次改的东西？」

31. **P10.5 引入的「导出 MIDI 读到旧编曲」（慢轨第二次扫描抓到，二分已钉死，修复中）**：既有用例 `e2e/clips.spec.ts:218` 确定性失败（`Expected: < 0.9 / Received: 0.9`，`--repeat-each=3` → **3/3 红**）。用例意图：把复制到 layer 1 的片段**编辑成静音**后，导出应只反映仍在发声的编曲。
    **二分（父代理实测，把 `seekMax` 打进两棵树量实际数字）**：`copied`（复制后的时长）两边都是 **0.9**；变的是「导出→重新导入」后的时长——**v1.112.0 = 0.5（通过）**、`.tmp/wt-p104`（v1.112.0+P10.4）通过、**`.tmp/wt-p105`（v1.112.0+P10.5）= 0.9（失败）**、主树同。⇒ **P10.5 引入**。
    **线索**：`smf.ts` 的 `parseMidi` 时长是纯音符驱动的 ⇒ 重新导入得到 0.9 说明**导出的文件里有延伸到 ≈0.9 的音符**，而界面上那个副本是空的 ⇒ **假设：导出读的是库里/快照里的旧 song，而不是当前活动编曲**（P10.5 动的正是 `src/midi/library.ts` 的 `put()`/`snapshot()`/`withSource()` 与 `PlayerPanel` 的导入分流；`src/state/*`、`src/midi/clips.ts` 自 v1.112.0 起未改动）。**已开轨道 `p105b` 修，要求先证实再修、不许改测试断言迁就。**
