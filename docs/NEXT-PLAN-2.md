...
| 11d | **P9.8 扩 arena + 采样修到接近 −60 dB**（用户已拍板扩 arena；按需增长、零分配不变、装不下优雅拒绝） | P9.7 | 中 | v1.114.0 |
| 12 | P10.4 工程管理 | — | 中 | v1.115.0 |
| 13 | P10.5 曲库合规 → **收官打 v2.0.0** | — | 中 | **v2.0.0** |
| 17b | **P12.6 SW 版本握手 + 修回滚后的横幅标签**（用户已批准动 SW 协议/i18n/首屏预算；排在 P10.4 之后） | P12.4 | 中 | v2.0.1 |
