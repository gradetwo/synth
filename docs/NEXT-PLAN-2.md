...
- **P9.1a 度量与门禁卫生**（零风险，先做）——按报告 `方案 A`：探测改到 **f32 精确频率**（或改 BH 频带求和、≥4 s、排除带按 8 bins 或 1%·k·f0）；每个场景 **pin 全部参数 + 清调制矩阵 + 稳定 400 块**；然后把阈值**收紧**（正弦 ≤ −105 dB、硬同步稳定 ≤ −70 dB，取代原来不稳的 −60 dB）。**不重录任何基线**（引擎未改）。
- **P9.1c 硬同步重启对齐**（新增，由 P9.1a 的实测发现立项）
...
- **初判机理**：重启点落在 2× 过采样网格上的**亚样本位置缓慢漂移**（周期数秒，量级与主/从振拍频相符），BLEP 有时对消、有时不对消 → 周期性「alias 爆发」。是**真实可听的缺陷**，且不在 P9.1b（只修普通振荡器）的范围内。
- **验收（比原来强）**：在 P9.1a 的新尺子下，**每一个** 4 s 窗都 ≥ **−60 dB**，且 ≥30 s 内的窗间离散度 **< 3 dB**；同时保留时域（周期相关、有界、无爆音）；三种波形与至少四个从振比值（含 1.41×、1.7×、11.3× 与非整数比）都要过。
- 输入材料：`/.tmp/hard-sync-drift.md`（P9.1a 的原始窗序列、两种尺子对照、拍频/比值消融）；完成后把结论折进 `docs/notes/hard-sync-aliasing.md`（把「已知缺陷」升级为「已修复 + 新验收」）。
...
| 顺序 | 批次 | 依赖 | 预估 | 版本 |
| :--- | :--- | :--- | :--- | :--- |
| 1a | P9.1a 度量与门禁卫生（零风险） | 调查结论 ✅ | 小 | v1.98.0 |
| 1b | P9.1c 硬同步重启对齐（新尺子下每窗 ≥ −60 dB / 离散度 <3 dB） | P9.1a | 中 | v1.99.0 |
| 1c | P9.1b 普通振荡器带限（音色改变，需重录基线） | P9.1a | 中大 | v1.100.0 |
