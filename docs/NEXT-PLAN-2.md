...
24. **采样「1C」方案：拿回带宽并把地板压到 −70 的路子，本批未走通（P9.8 后续，需专门一轮）**：P9.8 的现状（记作 1A）把内容压在自身 Nyquist 的 1/16，代价是**级 1/2（播放速率 1–4×）输出带宽收窄到 3–12 kHz**（根音及以下不变、≥5× 反而更宽）。把 `ν` 从 1/16 放宽到 1/4 并换 **16 抽头 BH4 sinc（1024 相位表 + lerp）**，理论上是唯一能**同时**保住 −70 dB 与全带宽的路，而且**低音确实大幅改善**（65 Hz −64.3 → −81.2）——说明插值核是好的，**问题出在级布局那一侧**。但实测门禁**红**：高音 1047/2093/4186 Hz 只有 **−47.1 / −45.7…−50.9 / −43.1**（现状是 −75.3/−73.8/−70.5）；更糟的是**结果依赖调用顺序**（同一进程、同一音、每次渲染前重导入同一份采样：`−47.13 / −66.99 / −67.51 / −52.71`，重导入不能消除）⇒ **不是池内容，是引擎状态或内存布局**。**门槛（必须先做）**：定位这个顺序依赖；然后把每级滤波截止按过渡带宽度下移、并让 loop 区避开抽取滤波器的边缘伪影。实现存在分支 **`p98-1c`（未合并，commit 5421198）**，含核设计与三条新 Rust 测试（`cargo test sampler` 15 passed）。**当前决定：接受 1A 的带宽代价**（已在 v2.0.1 发布），1C 另立批次。
