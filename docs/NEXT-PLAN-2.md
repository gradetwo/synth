...
43. **同音高双振荡器的相对初相随 note-on 变**（轨道 `pnoise` 途中发现，2026-09-15，**需拍板**）
   ...
   **偶发抵消基频**——6 次 note-on 实测 h2/h1 在 440 Hz 摆 **−25.5…+12.9 dB**、1568 Hz 摆 **−31.4…+19.0 dB**
   （单振荡器稳定 −6.0/−6.2 dB）。属和声平衡/可预测性的取舍，**不是缺陷**，但会让「同一个和弦每次弹都不一样」。
   **选项**：(a) 保持现状（去相关是有意的）；(b) 同设置时对齐相位（确定，但失去打散）；(c) 只在两振荡器设置相同时对齐；
   (d) 更慢的确定性漂移。**留给用户拍板，未立批。**
44. **推入限幅器时出现 `k·f0 ± 40 Hz` 侧带地板**（轨道 `pnoise` 途中发现，2026-09-15，低优先）
   用 `PATCH_GAIN`（id 78）把单声部推过限幅器 knee 后，BH-7 读 **−35.2 / −35.1 dB**（pg 6/8），增益轨迹峰-峰纹波
