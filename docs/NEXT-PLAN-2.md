...
> 本文档规划 **P9–P12 四个方向、23 个批次**（**另加用户 2026-09-14 新立的 P13「给其它 AI/LLM 的接口」5 个批次**，见 §三 P13）
...
| 维度 | 现状 |
| :--- | :--- |
| 版本 | **v2.1.6** 已上线并核对：部署时 `live D8m1K-ot == built D8m1K-ot`，父代理再独立 curl 校验 `local == live == assets/index-D8m1K-ot.js`，且线上 `sw.js` 与 `release/retained/2.1.6/sw.js` **逐字节相同**（sha256 `8c647dba…`、缓存 pin `gs1-84c5bb096892`）；`release/retained/` 保留 5 个（v2.1.6、v2.1.5、v2.1.4、v2.1.3、v2.1.2；v2.1.1 按窗口淘汰）。**`release/SHA256SUMS` 8 行 = 3 个本次工件 + 5 个 retained 快照，`sha256sum -c` 8/8 OK** |
| 门禁 | **整链 `npm run verify` PASS**（clippy/rust/build/unit/lint/wasm/ci/release/dist/budget/audio/presets×2/bench/dsp/dsp:2x/mcp/**worklet-protocol**/llm-docs）；单元 **69 文件 / 724 passed / 7 skipped** · Chromium E2E **156 passed / 10 skipped** · MCP 黄金会话两遍 sha256 相同 · `verify:ci`（含 timer 7 条 + worklet-protocol 新断言）与 `verify:llm-docs` PASS · **新增 `verify:worklet-protocol` 双向门禁**（文档↔代码 13 条）。⚠️ 合并版 verify 那次 `verify:audio` 计时**未判**（宿主 load 8.1，correctness-only）；`bench` 同类 |
| 体积 | dist **≤1550 KB**（v2.1.6 实测 **1536.0**；`workletmin` 把 AudioWorklet 资源从 35.1 → **15.0 KB**，**买回 19.7 KB**，否则本版会超线 5.2 KB）、首屏 JS **≤115 KB** gzip（实测 **114.5**）、CSS **≤21 KB**（20.4）、最大 WASM gzip **≤75 KB**（74.9）。⚠️ **首屏仍只剩 0.5 KB**，吃它的是「更新记录头条目长度」——下一个版本若照这个长度写 head 就要先买回空间；**worklet 那 19.7 KB 余量属于 dist 总额，不能当成首屏的余量** |
| 启动 / 性能 | v2.1.6 发布跑：首屏可交互 **834 ms** of [1113, 834, 1032]（预算 3200，FCP 788 ms）、fps idle **61.3 / 56.3**、playback **35.0 / 27.5**、graph-edit **61.3 / 53.8**（best / worst；playback 那次宿主较忙，worst 仍高于 20 的 floor） |
| 相容红线 | `test:dsp` **`rms 0.06147`**、`verify:dsp:2x` **`0.061703`**、`verify:presets` 与 `verify:presets:2x` 都是 **`91 presets unchanged · ABI 8`** 一字不动；零分配 `gs_alloc_violations()==0`；`PARAM_COUNT` **224**；ABI **8**。**v2.1.5 的 limiter 修法有逐 f32 位证明**；**v2.1.6 的 worklet 输入守卫对合法路径逐位不变**（四条红线未动）、**压缩只是把同一份代码去掉注释与空白**（另有一条「用构建产物渲染音频」的单测） |
| 已完成 | **P5–P13 全部交付**（P12.3 分享协作、P12.5 无障碍按用户决定不做）；债单批次 **P9.10 / p141 / p142 / p926 / ffx / trel** 全部交付（v2.1.1–v2.1.3）；**v2.1.4** 清 §一.8/.20⑨/.22/.23(截断半)/.25；**v2.1.5** 清 §一.1/.11/.12(根因)/.15/.29/.34/`player:153`/.39 残留/.42/.44/.45，并按用户决定关闭 §一.41/.43；**v2.1.6** 交付 **Groove 集成（按帧寻址事件）+ 协议契约与防漂移门禁 + 输入守卫 + worklet 压缩**，并完成 §一.48/.50/.51/.52 的 WebKit 信号诚实化与 **§一.47 的 PWA 假红修复**；源码里 **无 TODO/FIXME 残留** |
...
18. **nightly 默认子集在 1 fps 的 WebKit 上可能逼近 systemd 的 `TimeoutStartSec=3h`（P11.6 登记，未实测）**：默认子集从 8 个 spec 扩到 18（core+visual+audio），P11.6 按「不许为变绿缩子集」保留了它；第一次 timer 实跑后视情况决定是调 unit 的超时还是让默认走 `--core`（`scripts/systemd/gs1-nightly.service`）。
   **2026-09-15 进展**：①**可见性缺口已修（轨道 `nightlog`，v2.1.5）**——原来每个引擎的日志是 `spawnSync` 结束后才写盘，慢轨一次 `npm run nightly -- --engines=webkit,firefox --all` 跑了 **2 小时 12 分仍零输出**，只能中止；现在改成**边跑边落盘**（每 chunk 同步写、用户态零缓冲，SIGTERM/SIGKILL 都留下已写部分）并加「starting/finished」两行进度，判定语义（退出码 `code ?? 1`、`out` 仍是 stdout 后接 stderr、子集/重试/报告形状）一字未动。自证用假 `npx`：命令**未结束**时日志已有 `MARKER-A`，而改前同一时刻日志文件根本不存在。②**结论已出（轨道 `nightunit`）：3h「不够」，改跑 `--core` + 5h**。慢轨实测：Firefox 默认子集 **14.8 min 通过**（68 passed / **4 skipped 是宿主没有音频后端，不是绿**）；WebKit 默认子集 **>96 min 未完成**、`--all` **>132 min 未完成**、`--core` **>28 min 未完成**（历史完成值 22.7 min）⇒ 两引擎串行下界 **>111 min** 且 WebKit 无上限证据，超时后 unit 失败且**没有结果行**。**处置**：`ExecStart` 改成 `--engines=webkit,firefox --core --update`（8 个 spec：responsive/touch/text-fit/boot/fxgraph/share/drawer/theme），**理由写进 unit 注释**——CI 的 schedule `nightly` 作业仍对两个引擎各跑一遍 **`--all`**，所以 timer 走 `--core` **不是静默丢覆盖**，而是把「每天能跑完的核心子集」与「CI 上的全量」分工；`TimeoutStartSec` 3h → **5h**（依据：唯一完成的 22.7/14.8 min + 负载余量；注释明写「若 5h 仍不够，答案是再缩子集而不是加超时」）。`scripts/verify-ci.mjs` 新增 **7 条 timer 断言**把分工钉死：unit 存在、有 `ExecStart=`、**含 `--core`**、**不含 `--all`**、仍含两引擎、有 `TimeoutStartSec`、**且等于 5h**——改回默认子集或 `3h` 都会当场红（自证原文见轨道报告；踩过一个坑：第一版匹配整个 unit 文件，注释里解释用的 `` `--all` `` 字样把它自己判红了，已改成**只解析真实指令行**）。**边界如实记录**：5h 没有 WebKit `--core` 的上界证据，只有「约 13× 余量」。
...
39. **第三次全面回归（冻结 v2.1.0，2026-09-14）——两条确定性真红 + 一批「门禁在骗人」**（报告：`.tmp/sweep-v2.1.0-report.md`，589 行）
   ...
   **⑧ 覆盖缺口**：**WebKit 本轮未做**（用户指示 + 整包 42.7 min）；§一.38 的三条待处理项**已全部了结**：`roll:181` 的 `force` 缺口由 P13.4 修好、`player:153` 由轨道 `overflow` 查明并修、`flow:251` 由轨道 `flow251` 查明并修（两者都不是产品缺口，见第 38 条）。**仍开着的 WebKit 基线问题改记在 §一.48**（`flow:407` 的设备类别陈旧、`flow:246` 的贴边 flaky）。全部渲染/性能结论在**软件渲染**下取得（沙箱 tmpfs 盖住 `/dev/dri`，不是没有 GPU）。
...
47. ~~**`e2e/pwa.spec.ts` 的离线启动会在宿主负载高时假红，并因此中止过一次发布**（2026-09-16）~~
   **✅ 已修（v2.1.6，`2209d8f`）——它又挡了 v2.1.6 的发布一次，所以这条从「注册」升级成「必修」。**
   两次的失败点不同（v2.1.5：点完之后等 `.kbd-dock.open` 的 30 s 超时；v2.1.6：`click()` 本身
   `element was detached from the DOM, retrying` 直到预算耗尽），**同一个根因**：离线重载时 Service Worker
   接管页面会让启动遮罩被重新创建，于是「一次长点击 / 一次长等待」都在跟重渲染赛跑。
   修法正是本条原来写下的方向——**等引擎自己的信号**：
   ```ts
   ...
   ```
   即「遮罩还在就点它、遮罩消失就算引擎起来了」，后面的键盘坞仍是可见确认；**没有放宽阈值、没有加 retries**。
   自证：`--repeat-each=3` **12 passed**，离线那条 2.7 s。原文记录（当时的判定与数字）：
   离线重载后点「启动音频引擎」，`expect('.kbd-dock.open')` 的 **30 s** 硬等超时（当时 loadavg 6.4、整包并行）；
   同一条用例**单跑 4 passed、2.4 s 就过** ⇒ **假红，不是回归**。与 §一.39 的 `player:45` 同类：**贴计时线的用例在慢主机上会假红**。
53. **`npm run release -- --skip-verify` 会在「E2E 之前不重建 dist」这件事上骗人**（2026-09-16，父代理踩到，未做）
   `scripts/release.mjs` 的顺序是 **verify（含 build）→ E2E → perf → package（含 build）→ payload 门禁 → 部署**。
   用 `--skip-verify` 时 **build 被跳过**，而 `package` 的 build 在 **E2E 之后** ⇒ 如果树上的 `dist/` 是
   「上一次 build + 之后又改过源码/版本号」的状态，**E2E 跑的是旧产物**。本次的实际表现：
   v2.1.6 在 bump 版本之后直接 `--skip-verify`，`pwa.spec.ts` 的版本握手断言拿到 `2.1.5`（旧 dist 里的 SW）
   而 `APP_VERSION` 是 `2.1.6` ⇒ 红。**正确做法（已照做）**：bump 之后先 `npm run build` 再 `--skip-verify`。
   **建议修法**（未做）：`release.mjs` 在 `--skip-verify` 分支里于 E2E 之前**无条件跑一次 `npm run build`**
   （几秒钟的 Vite 构建 + wasm 拷贝），这样「跳过 verify」不再等于「跳过构建」。属独立小批，**登记待立**。
48. **WebKit 全 `flow.spec.ts` 里还有两条基线既有问题**（轨道 `flow251` 跑全 spec 时发现，2026-09-16）
   **✅ 已由轨道 `webkitsig` 查明并修（2026-09-16）；而且本条原来的两个归因都被实测推翻了，如实更正如下。**
   - **`flow:407/449`（手机两列 + 自适应）**：原写成「设备类别陈旧/`setViewportSize` 不给通知」——**错**。
     同路径探针读到 **`data-device = "phone"`、`cols = 2`、`horizontal = true`**，失败值也逐位复现
     `fill = 0.8459037162162162`。真因是**自适应受高度限制**：`384×590` 的图放进 530 px 高的画布区，
     `widthFit=(368−28)/384=0.8854`、`heightFit=(530−28−44)/590=0.7763`、slack=1.05 ⇒ `zoom=0.8151`
     ⇒ `fill = 384×0.8151/370`。旧判据量的是**引擎间那 8–10 px 顶栏高度差**（Chromium 同档 `wrap.h=540` ⇒ 0.8607，只高 1.1 个点），
     不是布局。**修法（(c)，阈值 0.85 未动）**：断言**受约束维度的填充** `max(fillW, fillH) > 0.85`
     （WebKit 0.846/0.904、Chromium 0.861/0.906），`cols=2`/`horizontal`/`zoom<100` 全留；
     改坏自证：把 `zoom` 钉成 0.32 下限 ⇒ 红（0.3496）。macOS 与 Linux 都绿。
   - **`flow:246/247`（每个节点 canvas 画了可见内容）**：原写成「阈值贴边」——**不准确**。真因是
     **单帧采样撞上了动画相位**（无 rAF 兜底时 headless WebKit 几乎不出帧、画面被冻住，所以以前看着“稳定”）：
     真实 harness 里 `fx2` 单帧读到 **94/94/92/100/90/56**。**修法**：12 × 200 ms 的 **lit 像素并集**（跨至少一个脉冲周期）
     ⇒ 三引擎最小值 **90（Firefox）/ 92（WebKit）/ 96（Chromium）**，**阈值仍 50**（1.8× 余量）；
     另加 `peak > 200` 的对比度下界（调色板最暗的 bypass 灰 = 452，黑盒 = 0）。改坏自证两组：
     整幅填 `#101010` ⇒ lit 0 红；填 `#303030`（R+G+B=144，**过得了墨迹线但过不了对比度**）⇒ `peak 144` 红。
     代价：该用例 WebKit 上 +2.4 s（16.5 → 25.0 s）。
49. **`player:11` 的「双击当前曲目续播」在 macOS 真 WebKit 上两次都红**（用户 2026-09-16 在 macOS 实测，待查）
    `e2e/player.spec.ts:11`：单点曲目自动播放（`:33` 断言 `.player-play` 带 `on`）与「停止」（`:37`）都通过，
    但 `:42` 的 `current.dblclick()` 之后 `.player-play` **始终没有 `on`**（20 s 超时，`Received string: "player-play"`，
    retry #1 同值）。产品侧逻辑（`PlayerPanel.tsx:1416`）是：`onDoubleClick` 先查 `performance.now() - lastSelect.current < 450`
    的守卫，再 `playing ? pause() : play()`；测试路径里首次单击的是**已经 current** 的曲目 ⇒ `onClick` 不更新 `lastSelect`，
    守卫理论上不该触发。**要查的是**：WebKit 里 `dblclick` 是否真的到达 React 的 `onDoubleClick`、
    `midiPlayer.play()` 是否被调用、以及 stop 之后 AudioContext 在 WebKit 上是否进入需要一次「新用户手势」的状态
    （单击能播、双击不能播 ⇒ 更像**手势/激活**问题而不是选择逻辑）。**本机 Linux headless WebKit 结果：`player.spec.ts` 8/8 通过（2.3 min）** ⇒ 不是所有 WebKit 都复现。
     **⚠️ 归因未定（2026-09-16 追加）**：用户那次 macOS 跑的是 **`e073df5` = 本仓库 `f120982` + 用户自己的新功能提交
     `feat(audio): frame-addressed note events (noteAt / noteOffAt)`**，所以这条红**可能是该功能引入的**，也可能仍是环境。
     **✅ 已查明并修（2026-09-16，与那个功能无关）**：用户在 **macOS 真 Safari 手动**验证「播放 → 停止 → 双击当前曲目」**能续播**
     ⇒ 产品侧没问题。随后用一份 A–E 五变体探针在 macOS Playwright WebKit 上定位（**headless 与 headed 读数完全一致**）：
     `locator.dblclick`（默认 0 ms 按压）**不产生 `dblclick` 事件**；`locator.dblclick({delay:60})`、`mouse.dblclick({delay:60})`、
     `dispatchEvent('dblclick')` 都能触发。**真因在套件自己的交互层**：`e2e/interact.mjs` 把 `dblclick` 实现成
     `mouse.click(x, y, { clickCount: 2 })`——**一次 down/up 对**，而 WebKit 不会因此派发 `dblclick`。Chromium 会派发，所以只有 WebKit 套件看见；
     而**本机 Linux headless WebKit 的 `player.spec.ts` 却是 8/8 全过**（同一实现）⇒ 这是**引擎/版本差异**（macOS 那份 WebKit 与 Linux 那份对
     `clickCount: 2` 的处理不同），所以本机复现不了、只能靠 macOS 的读数定位。**修法**（`ba25496`）：`interact.dblclick` 改成解析同一个
     帧无关坐标后发**真正的 `mouse.dblclick`**（两对 down/up、按压 60 ms，调用方仍可覆盖 delay）；`mcp/ui/interact.test.mjs`
     原本把**错误实现**钉成了契约（`expect(mouse.click).toHaveBeenCalledWith(…, {clickCount: 2})`），已改成钉真双击并断言**不再**用 `click`。
     **待用户在 macOS 复跑确认**：`npx playwright test --project=webkit --workers=1 --retries=0 e2e/player.spec.ts -g "lists the built-in songs"`。
    这条不在任何既有 `--core` 子集里，所以之前的 WebKit 慢轨从没覆盖到它。
50. **`pwa:105` 的离线重载是 Playwright WebKit 的引擎内部错误，应改成具名跳过而不是留红**（用户 2026-09-16 macOS 实测，未做）
    `page.reload: WebKit encountered an internal error`——**Linux headless WebKit 与 macOS 真 WebKit 都复现**（两次尝试同错）。
    同 spec 的另外 3 条（重定向 shell、SW 控制、版本握手）都绿。⇒ 这不是产品缺口，也不是计时假红，
    而是**引擎/驱动层在 `context.setOffline(true)` + `reload` 上的限制**（§一.38 早就把 `pwa:100` 记成
    「WebKit encountered an internal error（引擎差异/已知 flaky）」）。**建议**：在这条用例里按 `browserName === 'webkit'`
    **显式跳过并打印原因**（照 `e2e/audio-host.ts` 的既有做法：原因上屏、终端可见），把「离线启动」的覆盖面
    明确留给 Chromium（那边它一直真跑，只是在高载下有 §一.47 的 30 s 假红）。
51. **`audio:148`（MPE 开关）在 macOS WebKit 上 flaky 一次**（用户 2026-09-16 实测，低优先）
    第一次 20 s 内找不到 `[data-setting="mpe"] input`（`.audio-settings.open` 里没有该行 ⇒ 面板还没开完/行还没渲染），
    retry #1 1.8 s 就过。与 §一.47 同类（**等面板/引擎的异步状态却用固定超时**），不是产品问题。
    建议修法与 §一.47 合并处理：把那类「打开抽屉后立刻找行」的等待改成等面板自己的就绪信号。
52. **`responsive:35` 的「触摸目标 ≥40 px」在 WebKit 仿真里根本测不到**（用户 2026-09-16 在 macOS 真 WebKit 上跑 nightly `--core` 发现，待定）
    同一子集 39+ 条里 **33 passed / 6 failed**，而 6 条失败**全是同一个断言**：`responsive.spec.ts` 的
    六个设备档（iPhone portrait/landscape、iPad portrait/landscape、iPad mini portrait/landscape）都在
    `:76 expect(cramped).toEqual([])` 上红，`Received: ["82x19","95x19","152x19","53x19"]`
    ——设置抽屉里有四个文字按钮只有 **19 px 高**，判据要求 `button/select` 至少 40×40。
    同一子集里 `touch` / `fxgraph`（23 条全绿）/ `text-fit` / `boot` / `share` / `drawer` / `theme` 在 macOS WebKit 上**都通过**。
    （当时的假设与修法方向已由下面的实测结论取代。）
    **✅ 轨道 `webkitsig` 的结论（2026-09-16）**：机制**部分证实**，但**本机 Linux WebKit 完全不复现**
    （`responsive.spec.ts` **6/6 绿**，22.0/34.1/24.3/30.9/18.6/58.4 s）。**已证实**：40 px 触摸目标确实全住在
    `@media (pointer: coarse)` 下，而 **`(pointer: coarse)` 就是总开关**——`hasTouch` 开关法对照：Linux WebKit 在
    `hasTouch/isMobile` 下 `pointerCoarse = true`（`cramped` 为空），**关掉 `hasTouch` 立刻变成 16 个 28/30 px**。
    **未证实、需要 macOS 侧核实的是「那 4 个 19 px 元素是什么」**：本机 no-touch 形态是 **16 个 28/30 px**（数量与高度都不同），
    且 `gs1.css` 基线里 `.roll-btn` 是 28 px、`.d-theme-btn` 是 30 px，**没有任何规则给出 19 px** ⇒
    「macOS 的 `coarse === false`」单独**解释不了**那个读数。
    **采用的修法（保守，掩盖不了真缺口）**：页面里读 `matchMedia('(pointer: coarse)').matches`，为 false ⇒ 打印读数 +
    `test.info().annotations` 标注，**只跳过「触摸目标」这一条断言**（5 个 section / 抽屉宽度 / 无横向溢出 / 手机 chrome /
    键盘等全部保留）；且**只允许 WebKit 走跳过分支**（`expect(coarse || browserName === 'webkit')`）——
    Chromium/Firefox 读不到就**直接失败**，覆盖不会悄悄消失。40 px 与 `cramped` 算法**一字未改**；改坏自证：
    把 44 px 改成 20 px ⇒ Chromium `responsive -g "iPhone portrait"` 红（16 个 cramped）。
    **待办（macOS 探针，已请用户跑）**：打印那 4 个 cramped 元素的 `tag/class/data-act/文本/computed min-height`
    \+ `matchMedia('(pointer: coarse)').matches`。**若 `coarse === true` 而那 4 个元素本该命中 coarse 块 ⇒ 真缺口**，
    叫停跳过、走产品侧补 40 px；若 `coarse === false` ⇒ 属环境差异，保守跳过已够。
    **✅ 探针已跑（2026-09-16，macOS 26.6 真 WebKit），答案是「真缺口，但不在我们以为的那一层」**：
    `matchMedia('(pointer: coarse)') === true` **且 CSS 引擎自己也认为 coarse**（注入只在 `@media (pointer: coarse)`
    里生效的规则，computed 命中）——所以那段注释里「macOS 报 false」是**错的**，已改。四个元素是设置抽屉里的
    **4 个 `<select>`**（场景/按键分配/调律/力度曲线），读数逐个是：`minH=18px`、`minW=44px`、`font=11px`、`pad=0px`、
    `appearance=auto`；**同排每一个 `<button>` 都是 44×44**。也就是说 coarse 块**命中了**（`min-width`/`font-size` 生效），
    但 macOS WebKit 把 **原生外观的 `<select>` 按 NSPopUpButton 排版，忽略作者的 `min-height` 与 `padding`** ⇒
    44 px 的触控承诺在这四个控件上不成立。**未证实**：真实 iPad/iOS（同族引擎、同样原生 select）是否同样受影响。
    **✅ 用户选 ②，已交付并确认（`57f580d`，2026-09-16）**：每个 select 外套 `<label class="select-target">`，
    **fine 指针下 `display:contents`（不产生盒子：wrapper 实测 0x0、select 仍 104x36，桌面布局逐像素不变）**，
    coarse 下 `inline-flex` + `min-height/min-width:44px`；**没有** `appearance:none`，**也没有** `showPicker()`
    ——BCD 里 Safari 只有 preview/带 flag，本机 Linux WebKit 实测 `typeof select.showPicker === 'undefined'`，
    且 `preventDefault()+showPicker()` 会把 edge tap 的 click/focus/value 一起打断（同页负对照已留证）。
    `responsive.spec.ts` 改为量 wrapper、排除被包住的 select。本机把 macOS 形状造出来验（把 select 钉成 19px）：
    旧测量集 `cramped=[73x19,87x19,143x19,44x19]`，新测量集 `[]`，wrapper `73x44/87x44/143x44/44x44`（Chromium）、
    `79x44/92x44/149x44/50x44`（Linux WebKit）。**macOS 侧确认**：gate 包（tree `8df19e6`）WebKit `--all` 里
    `responsive` **六条全绿**，那四个 19 px 读数不再出现。**仍未证实**：wrapper 空白处的点击在 macOS/iOS WebKit 上
    能否**真的弹出原生选择器**（Linux 只证明了转发 + click/focus/键盘签名；iOS 只有 label 原生转发这一条路）。
54. **帧无关点击在 macOS WebKit 上会偶发不投递**（2026-09-16，慢轨全量 + 用户 macOS A/B 实测，**未修**）
    `--all` 里 `boot`（等 `.audio-settings.open` 120 s 超时）、`guide`（EN 点击 120 s 超时）、`fxgraph`（切回实例 2 时
    `kind(0)` 读到 `delay`）三条**在 Linux WebKit 上各跑两遍全绿**，归档快照证明**click 的 onClick 从未执行**
    （boot：页面里根本没有 `.audio-settings` 节点；guide：语言仍中文）。**A/B 把它定性了**：同一段重放，
    `e2e/interact.mjs` 的帧无关点击在 `switchTo(1)` 卡住（`.settings-drawer .d-close` 点完抽屉仍 `.open`，20 s 超时），
    而 `GS1_E2E_FRAME_FREE_CLICKS=0`（Playwright 原生点击）**3 passed (12.5 s) 全过**；同一次默认运行里
    「打开音频设置」「EN 切换」却是过的 ⇒ **偶发**，不是全坏。
    **✅ 两条都拿到了（2026-09-16，用户 macOS 第三轮）**：
    ①**真实读数全绿**：`GS1_E2E_FRAME_FREE_CLICKS=0 npx playwright test --project=webkit e2e/boot.spec.ts e2e/guide.spec.ts e2e/fxgraph.spec.ts`
    ⇒ **23 passed / 0 failed（47.4 s）**——**这三条 `--all` 红完全是测试基建的锅，产品在 macOS WebKit 上是好的**；
    ②**失败那次的事件列表抓到了**（`clicks-x3.log:22`，新版探针第一遍就复现、3 遍全失败）：
    `"pointerdown->button.d-close\"✕\"", "pointerup->button.d-close\"✕\""` 而**没有 `click->…`**，
    紧接着 `{"drawerOpen":1}`——**down/up 都打在同一个选择器上，但引擎根本没派发 `click`**；同一串里前一对
    `button."1"` 的 down/up/click 是完整的。根因落在 `e2e/interact.mjs` 的
    `page.mouse.click(x, y, { delay: options.delay })`——**默认 0 ms 按压**，与 §一.49 里 `dblclick` 踩的是同一个坑
    （那次已改成 `delay: 60`，`click` 还没有）。修法（轨道 `frameclick` 实施中）：补按压延时，**并让动词自检**——
    派发前装捕获阶段监听器，没收到落在目标（或其后代）上的 `click` 就重解析、重派发，直到既有 deadline 才失败。
    **这是「围绕真实手势的重试」，不是放宽断言**：真到不了就照样超时，错误信息写明缺的是哪个事件。
    本机（Linux）复现不了这种漏派发，**macOS 复验仍待用户**。
55. **`player` 的「选中曲目自动播放」只在整套 `--all` 里偶发失败，单独跑复现不出来**（2026-09-16，待查）
    归档：macOS WebKit 与 Firefox 都红（`:32 expect(play).toHaveClass(/\bon\b/)`，失败瞬间 `aria-label="播放"` ⇒
    应用自认没在播），而**同 spec 的兄弟用例**（`loops an A/B region`）做同样两步却通过。macOS 探针**单独跑通过**
    （两变体都过：`player-play on` / `暂停` / `seek` 每秒前进，WebKit 与 Firefox 各一次）⇒ **不是确定性产品缺陷**，
    也不是「默认曲目已选中导致点击成空操作」（探针证明默认 current 是「琶音（练习）」、打开面板时并未在播）。
    **✅ 已收窄成两条各不相同的事实（2026-09-16，用户 `--repeat-each=5`）**：8 条 ×5 = 40，
    **5 failed / 35 passed，且 5 次全是同一条**——`:11` 在 **`:42`**（停止后**双击当前曲目续播**）红，
    `Expected pattern /\bon\b/`、`Received "player-play"`：**这是确定性 5/5，不是 flake**；而归档那次红的
    `:32`（点曲目自动播放）这回 5/5 都过。两者是不同的断言，得分开处理。
    **最可能的机制（已在源码里定位，待 macOS 实测确认）**：`PlayerPanel.tsx:1416` 的 `onDoubleClick` 有一条
    **450 ms 守卫**（`performance.now() - lastSelect.current < 450` 就直接 `return`），而 `lastSelect` 只在
    **选中另一首**时更新——测试在 `:30` 选了致爱丽丝，其后几条断言加一次 stop 点击若在 **450 ms 内**完成，
    `:41` 的双击就被这条守卫**有意吞掉**，`:42` 必然等到 20 s 超时。**本机为什么必过**：这台机器（且在负载下）
    实测「选中 → 双击」间隔 **8240 ms**，Linux WebKit 更慢（每次点击以秒计），都远在窗口外；macOS M2 上这些步骤
    快到可以落进窗口。**✅ 已证实并按测试侧修好（2026-09-16，用户 macOS 探针）**：`e2e/_probe-dblguard.spec.ts`
    在 macOS WebKit 上直接量到该间隔——**帧无关点击 271 ms / 原生点击 86 ms，两者都被吞掉**（`playHasOn=false`）；
    把间隔拉到 **970 ms / 818 ms** 后 `playHasOn=true`。同时
    `GS1_E2E_FRAME_FREE_CLICKS=0 --repeat-each=5 e2e/player.spec.ts` **仍然 5/5 红在同一条 `:42`**
    ⇒ **与 §一.54 的交互层无关**，就是**测试跑得比人快，撞上了产品那条有意的 450 ms 去抖**。
    修法（测试侧，`53aa410`）：双击之前显式等过该窗口（`e2e/player.spec.ts` 加 `waitForTimeout(500)`，
    注释里带上 86/271 与 818/970 四个实测数与去抖的出处）。**产品不改**——450 ms 去抖是有意的，
    用户早已在真 Safari 手工确认过产品行为正确。
    **✅ 修后在同一台 macOS WebKit 上复验（2026-09-16）**：同一条命令
    `--project=webkit --repeat-each=5 e2e/player.spec.ts` ⇒ **40 passed (1.1 min)**（修前 5 failed / 35 passed (2.7 min)，
    且 5 次失败每次都在烧 20 s 断言超时，这就是整包耗时从 2.7 min 掉到 1.1 min 的原因）。**仍未证实**：归档那次 `:32`（点曲目自动播放）只在整套里偶发，
    单独跑与 5 次重复都过，机制未明（怀疑与前序 spec 的宿主负载有关）。

56. **WebKit/Safari 上导出的是静音**（2026-09-16，gate 包实测发现，**用户决定暂不查**）
    `e2e/export.spec.ts › exports a WAV that really contains audio`（本轮新增的断言）在 macOS WebKit 上
    **两次运行都收到 `peak = 0`**（`Expected > 1000`）；同一条 `renderPatchToBuffer` 路径在 Chromium 上峰值
    **27034/32767**。**MP3 那条一直是绿的，是因为它只断言「下载到了、体积 > 300 KB」——静音的 MP3 同样满足**，
    所以这个缺陷此前完全不可见。**影响**：Safari/WebKit 用户导出的音频文件是无声的（数据丢失级）。
    **待查方向（未做）**：WebKit 的 `OfflineAudioContext` + AudioWorklet 是否产出；或 `suspend` 路径在 WebKit 上
    让 worklet 空转。**用户拍板：本轮先不查**，登记为已知缺口。
57. **Node 26 的实验性全局 `localStorage` 盖掉 jsdom 的，15 条 vitest 在 macOS 上必红**（2026-09-16，gate 包实测）
    gate 包在用户机器（**Node v26.8.2**）上 `npm run verify` 死在 `npm run test`：`src/state/store.test.ts`
    **15 failed / 25 passed**，全是 `localStorage` 读/写/clear 处的 `Cannot read properties of undefined`。
    决定性证据是用户终端里那行 `ExperimentalWarning: localStorage is not available because
    --localstorage-file was not provided.`——Node 26 默认注册实验性全局 `localStorage`，**没给该 flag 时它是
    undefined，并且把 vitest jsdom 环境提供的那个盖掉了**。**本机 Node v22.22.3 无此全局**：`npm run test`
    **748 passed / 0 failed**。**✅ 已修（`56ed998`，2026-09-16）**：`vitest.setup.ts` 在全局缺失/不可用时装上 Storage API
    （Map 实现的垫片；有 window 提供的就用 window 的，可用的全局一律不碰）。**两个候选都先被实测否掉**：
    「把 `window.localStorage` 拷回全局」无效（此环境下 **`window` 就是 `globalThis` 的别名**），
    「删掉影子全局」也没东西露出来（两者都在 Node 22 + 模拟器上量过）。
    **自证**：用 `NODE_OPTIONS=--require .tmp/simulate-node26.cjs`（把两个全局定义成 `undefined`，即 Node 26
    未给 `--localstorage-file` 时的全部行为）在本机复现出**同样的 15 条失败**，装上垫片后 `store.test.ts` +
    守卫测试 **2 files passed**；不加模拟器整库 **744 passed / 7 skipped**（Node 22 上 no-op）。
58. **macOS 27 下 Playwright 的 Firefox 起不来：`Could not find profile folder.`**（2026-09-16，环境，未修）
    gate 包在用户机器上 `firefox all` **0 passed / 147 failed（58 s）**，每条都是
    `browserType.launch: Failed to launch the browser process`，浏览器自身 stderr：
    `*** You are running in headless mode.` / `Could not find profile folder.` / `exitCode=1`。
    **`TMPDIR=/tmp` 复现同样失败**（`-profile /tmp/playwright_firefoxdev_profile-Y6FBZX`）⇒ 不是 TMPDIR 不可写。
    时间线：这批中间用户把系统从 **25.6.0 升到 27.0.0**（三份 gate 包的 `host.osRelease` 都是 27.0.0，
    而唯一跑完 Firefox 的那份归档是 25.6.0）。**待试**：`npx playwright install --force firefox`；
    `xattr -dr com.apple.quarantine <Nightly.app>`；手工用同一条命令行启动看真实报错；若确认是 macOS 27
    与该 Nightly 构建不兼容，则需升 Playwright（随之换 Firefox 构建）。**覆盖面不丢**：Firefox 在本机 Linux
    一直跑（player/roll/export 本轮均绿），CI 的 schedule `nightly` 仍对两个慢引擎各跑一遍 `--all`。
59. **WebKit 上另外三条：自检既是解药也是病因**（2026-09-16，**已在 Linux WebKit 上取证**）
    gate 包（tree `8df19e6`）里三条失败：`flow.spec.ts` 手机键盘 120 s 超时、`preset-audition.spec.ts`
    预设名停在 `SEM Notch Drone`、`roll.spec.ts` 垂直拖动试听期望 `D2` 得到 `—`。
    **取证方法**：自检默认只在 Darwin 开，但 `GS1_E2E_FRAME_FREE_SELFCHECK=1` 能在 Linux 上跑**同一条代码路径**，
    于是做了开/关两轮对照（Linux WebKit，`--workers=1 --retries=0`）：

    | 用例 | 自检关 | 自检开（= Darwin 的路径） |
    | :-- | :-- | :-- |
    | `flow` 手机键盘 | ✘ 失败 2.1 min（`locator.evaluate: Target page … closed`） | ✓ **5.8 s 通过** |
    | `preset-audition` | ✓ 1.9 min | ✓ 1.6 min |
    | `roll` 垂直试听 | ✓ 51.6 s | ✘ **失败** 46.5 s（`toBe` 不匹配） |

    **结论**：①`flow` 那条是**真的点击被吞**，自检把它修好了——自检的价值有了独立于 macOS 的证据；
    ②**`roll` 那条是自检引入或暴露的**（关绿开红），而 Darwin 上自检恒开，正好解释了 macOS 的那条红。
    **待查**：自检在按压前后各多一次 `evaluate`（装/读录音器）加 30 ms 按压时长，对**拖动**手势的取样时机有影响
    （`roll:181` 正是纵向拖动试听）；要定位是哪一个，并让自检对拖动类手势无害（拖动不走自检，或把录音器的装/读
    挪出拖动窗口）。③`preset-audition` 在 Linux 两态都绿 ⇒ 更像 macOS 专有，仍待 macOS 侧读数。
...
**P11.7 慢轨打包：在快机器上跑、把结果带回来** ✅ **交付（未发版，基础设施）**
- **要解决的问题不是「跑」而是「归因」**：本机 WebKit 核心子集 **>96 min 未完成**、`--all` **>132 min 未完成**（见 P11.6 与 `docs/notes/nightly.md`），所以慢轨必须在别的机器上跑；但如果只拿回一行 passed/failed，归因还得从头再来一遍。
- **`scripts/slow-lane-pack.mjs`**：每个 run **独立端口**（`base+i`）、**独立 JSON 报告**（`PLAYWRIGHT_JSON_OUTPUT_NAME`）、**独立流式日志**（边跑边写，`tail -f` 有效），超时**杀整个进程组**（SIGTERM，10 s 后 SIGKILL）；单个 run 红掉**不中断**打包，继续跑下一个引擎，最终退出码 1。**引擎串行是设计的一部分**：Playwright 在每个 run 开始时清空 `test-results/`，所以脚本在每次 run 结束后**立刻**把它按 run 名复制出来——并发跑就会互相覆盖失败上下文。
- **产出**（打成 `.tmp/slow-lane/<label>-<时间戳>.tar.gz`）：`SUMMARY.md`（内核 × 计数表：passed/failed/flaky/skipped/wall + 每条失败的报错截前 12 行，已剥 ANSI）、`environment.json`（宿主 CPU 型号/核数/内存、node、playwright、包版本、git HEAD、**dist 入口 JS 名与 `index.html` 的 sha256**、每个 run 的退出码）、`logs/`、`reports/*.json`（SUMMARY 就是由它生成的）、`test-results-<run>/`、`manifest.sha256`。
- **子集常量抽出单一来源** `scripts/lib/e2e-subsets.mjs`（core 8 / nightly 18，`subsetFor()` 对未知名字返回 `null` 而不是静默退化成默认子集），`scripts/nightly-e2e.mjs` 改为 import、删掉本地清单。自证：`--dry-run --core` 打印 **8** 个文件、默认打印 **18** 个。
- **自证（快轨，本机跑得完）**：`--self-test` 用 chromium + `e2e/smoke.spec.ts` 走**完整管线**（build 复用 → run → 采集 → 归档），实测 **1 passed / 6.5 s**，并断言归档内 log、report、summary、environment、manifest、tar 六项齐全；**失败路径**用临时 probe spec 实测（webkit **32 s** + firefox **9 s** 双红）⇒ SUMMARY 含 2 条失败、报错给出 `Expected/Received`、退出码 1、两个 run 各自的 `test-results-<run>/error-context.md` 都在。过程中修掉一个真 bug：`copyTree` 与 Playwright 收尾删除 `test-results/.last-run.json` 竞态（`ENOENT` 改为跳过，不是结果就丢）。
- 文档 `docs/notes/slow-lane-pack.md`（为什么需要、怎么用、参数表、包内布局、**拿回来看什么**），`docs/DSP-GUIDE.md` 的 notes 索引行加上它；`npm run slow-lane` / `npm run slow-lane:self-test`。
- **未做（如实登记）**：本批**没有**真跑一次 WebKit/Firefox 慢轨来做验收——本机跑不完正是这个脚本存在的理由，所以「慢轨结果本身」仍是待办（由用户在快机器上执行后带回）。归档的回传也没有自动化，人工搬文件是设计的一部分。

**P11.8 第一次跨引擎全量读数（P11.7 的第一次真实交付）** ✅ **交付（未发版）**
- **第一次有人把 WebKit 与 Firefox 的整个套件跑完**：用户在 macOS 26.6 / M2 Max / 12 核 / 32 GB 上跑
  `--engine=webkit,firefox --subset=all`：**WebKit 132 passed / 13 failed / 11 skipped / 595 s**、
  **Firefox 139 / 6 / 11 / 581 s**（各 145 条）。同一命令在本机 WebKit **>132 min 未完成**。
  **结论：慢的是机器，不是用例**——除超时用例外，两个引擎上最慢的**通过**用例只有 **16.5 s**。
  `manifest.sha256` 校验 **28/28**，`environment.json` 钉住了 git HEAD 与 dist 入口（正是加它的目的）。
- **19 条失败全部分诊完毕**，按性质分五类，其中**两条是真产品缺陷**（都已修）：
  1. **测试自身（3 条，已修 `84de040`）**：`roll` nudge（音符是 `pointer-events:none` 的 **0.75 px** 细条，
     点哪个由应用的 x→音符映射决定，Chromium 落 C5@0.90 s、WebKit/Firefox 落 G4@0.60 s，而编辑行为逐字相同 ⇒
     硬编码 `'0.90 秒起'` 是内核指纹，改成断言**增量**）；`takes` 并集（首遍弹了床音自己的音高 62，而
     `TAKE_RESTRIKE = 0.05` 会把同音高 ≤50 ms 的音**替换**掉，起点是「点录制→按键」的墙钟延迟：
     WebKit 0.033 s ⇒ 1 个音、Chromium 0.12–0.36 s ⇒ 2 个音，**同一代码只有起点不同计数就翻**，首遍改弹 63）；
     `audio` 预载（页面读数无法区分「服务器没给 `application/wasm`」与「这条响应没有 body / 没有可读头」——
     304 与不透明 SW 预取都会读成 `""`，改成**直接问源站**要 MIME + 保留面板 `流式编译` 的端到端断言）。
  2. **Firefox 全平台导不出音频（已修 `849d010`）**：Gecko 没有 `OfflineAudioContext.suspend`
     （MDN BCD `version_added: false`，bug 1265406），`renderPatchToBuffer` 在第一帧之前就抛
     `TypeError: o.suspend is not a function` ⇒ **MP3 与 WAV 都导不出来**；`.mid` 走另一条路所以套件看起来「只是偶尔红」。
     修法是**守护路线**：能 suspend 的引擎（Chromium/WebKit）保留原排程，Gecko 走 worklet 的帧寻址预填
     （`processorOptions.notes`，与 `routes` 同一条通道），**并实测两条排程不等价**——suspend 把 note-on 落在
     `start` 之后的**下一个 128 帧量子边界**，帧寻址精确到帧，差 `(128 − atFrame % 128) % 128` 帧（0–2.88 ms，只会更早），
     所以**不**把既有用户的导出静默重新对齐。等价性用**逐样本 SHA-256** 证：单音与 12 音带 pan 乐句改造前后**逐位相同**。
     校验器抽成唯一的 `timedEventFrom`（消息路径与预填共用，不留第二个更弱的）；预填**有意**绕过
     `MAX_SCHEDULED_EVENTS`（那是防实时宿主漏事件的上界，截断会静默切掉歌曲开头），并有 1200 事件不丢的单测。
     顺带**补上原先完全没有浏览器覆盖的 WAV 导出**：新增 E2E 断言 RIFF 头 + **不是静音**（实测峰值 27034/32767，
     阈值 1000 有 27 倍余量；把阈值抬到 999999 会红，证明断言不空洞）。三引擎：MP3/WAV 用例 chromium/webkit/firefox 全绿，
     Firefox 的 MP3 单条 **4.1 min 失败 → 1.0 min 通过**。
  3. **钢琴卷帘 marquee 框选（已修 `907ec12`）**：选中集非空会渲染选择条与 inspector，把 `.roll-grid` **下推 90 px**
     （WebKit 实测 top 在 189↔279 震荡），而 marquee 的**起点**在 pointerdown 冻结、**终点**每次读 live grid 盒 ⇒
     同一手势两端两套坐标系，bar 一出现框覆盖的音高就跳 4–5 个半音：清空选中 → bar 消失 → 恢复，来回震荡，
     用户看到的是「框住音符却选不中」。修法是给 `beatAt`/`pitchAt` 加可选原点（默认仍读 live 盒，非手势调用者行为不变），
     手势在 pointerdown 捕获原点并全程复用。修后同一手势读数由
     `height 20,40,60,60,80,80,20,20,140 / sel 0,0,0,0,1,1,0,0,3` 变成
     `height 20,40,40,60,80,100,100,120,120 / sel 0,0,0,0,0,2,2,2,2,3`；WebKit 该用例 3/3 绿（34–54 s，修前失败那次 1.5 min
     里有 20 s 是耗尽的断言预算）；Chromium `roll.spec.ts` 18 passed；视觉 10 passed **无基线移动**。
     修前 flake 本会话实测 **1/2**（不是先前以为的 2/2），如实记录。
  4. **macOS 专属、待第三轮（见 §一.54/55）**：`boot`/`guide`/`fxgraph` 三条在 Linux WebKit 各跑两遍全绿，A/B 显示
     **`e2e/interact.mjs` 的帧无关点击是嫌犯**（同一重放：默认红、`GS1_E2E_FRAME_FREE_CLICKS=0` 原生点击 3 passed）；
     `player` 那条**单独跑就通过**（macOS 两个引擎的探针都过）⇒ 只在整套里偶发。
  5. **触控目标（见 §一.52）**：不是「`(pointer: coarse)` 读不到」，而是 macOS WebKit 的**原生外观 `<select>`
     忽略 `min-height`/`padding`**（computed 18px，而 `min-width`/`font-size` 生效）；四个 button 全是 44×44。修法待拍板。
- **本批自己产出的工具**：`scripts/slow-lane-pack.mjs` 修掉一个标签 bug（`--subset=all` 原显示 `0 file(s)`，
  现为 `whole suite (no file filter)`）；三条可复用教训写进 `docs/notes/webkit-testing-pitfalls.md` §6
  （**盲点击的命中是亚像素事实**、**别在无意中测一个定时边界**、**重验证响应没有表示元数据**）；
  `docs/notes/compat.md` §5 里「Firefox 的 MP3 只是渲染慢」这条**错误假设**已更正（它不是慢，是立刻抛错）。
- **红线与门禁**：`test:dsp rms 0.06147`、`verify:dsp:2x 0.061703`、`verify:presets`/`2x` **91 presets · ABI 8** 全部未动；
  `test:rust` **239 passed**；`vitest worklet-processor` **31 passed**；`verify-dist` PASS（**1536.7 KB**）；
  `verify-budget` PASS（1536.7/1550、首屏 JS gzip **113.9/115**、CSS 20.4/21、wasm 74.9/75）；
  `verify-worklet-protocol` PASS。
- **未证实（如实登记）**：WAV 那条新增 E2E 没做「改坏必红」的重建往返（同一代码路径的红→绿证据是 MP3 那条），
  只用「把阈值抬高会红」证明断言非空洞；macOS 第三轮结果；**真实 iPad/iOS 是否同样忽略 select 的 min-height**。

**P11.9 慢轨打包的第二次真实交付：门禁也能在快机器上跑** ✅ **交付（未发版）**
- **打包器长出 `--steps` 模式**：跑仓库自己的 npm 脚本（`verify`/`e2e`/`test:dsp`…）以及
  `pw:<project>:<subset>`（如 `pw:webkit:all`）形式的一次引擎运行，**采集/SUMMARY/manifest/tar 与慢轨模式完全共用**。
  于是「门禁 + Chromium + 跨引擎全量」可以一条命令、一个包带走（`npm run gate-pack:full`）；
  而 `--subset=all` **只是**慢轨（不含 Chromium、不含 `verify`）这件事已写进 `docs/notes/slow-lane-pack.md`。
- **两次真实运行各暴露一个打包器自己的缺陷并已修**：①steps 模式原来**从不 build**，而 `verify` 在用户机器上
  0 秒就死在 `'cargo': Bad CPU type in executable`（装的是错架构的 Rust；后已修好，Rust **239 passed**），
  于是三个浏览器步骤全在服务一小时前的 `dist/`——症状是 Firefox 报出一个树里早已修掉的
  `o.suspend is not a function`；现在**有浏览器步骤就先 build**（单独一行）、`dist` 比 `src` 旧时告警。
  ②用户 `e2e/` 里残留的 `_probe-*.spec.ts` 被每个 Playwright 步骤收集、制造 2 条假失败；现在**直接拒绝启动**
  （`--allow-probes` 可绕过）。
- **本轮修掉/确认的**（细节见 §一.52/54/55）：44px 触控目标（`57f580d`，macOS WebKit `responsive` **6/6** 确认）、
  交互自检与重试（`5ce9994`，macOS `--all` 从 13 条失败降到 **4** 条，`guide`/`i18n`/`param-range`/marquee/player 全绿）、
  `player:42` 的 450 ms 去抖（`53aa410`，macOS `--repeat-each=5` **40 passed**），以及本轮早先的三条测试修复与
  Firefox 导出修复（`84de040`/`849d010`）。**Chromium 快轨**在用户机器上 **157 passed / 0 failed**。
- **最终门禁**：本机 Node 22 上 `npm run verify`（数字见提交后的报告）；lint 曾因交互自检遗留的两处错误而红
  （`1afc541` 修好）。
- **仍未绿/未证**：§一.56（WebKit 导出静音，用户暂不查）、§一.57（Node 26 的 localStorage，15 条）、
  §一.58（macOS 27 的 Firefox 启动）、§一.59（WebKit 另三条）。
...
| 30 | **v2.1.5 六条轨道（多线并行模式）** ✅ 视觉基线接 CI（schedule + 双向断言）、fps 最差窗、工具链版本先于 clippy、`SHA256SUMS` 覆盖全 retained（**发布后实测 8 行 `-c` 全 OK**）、参数 id 段护栏（**前提被证伪**，不重排）、`MASTER_TUNE` 双应用（Rust + 真 wasm 门禁，默认 0 逐位不变）、**限幅器 `k·f0±40 Hz` 侧带**（机制：采样相位图案；修后 −89 dBc、逐位不变）、窄屏陈旧设备类别（一行 CSS）、nightly 日志边跑边落盘、并行端口假红修复（父代理集成窗口） | — | 中 | ✅ **v2.1.5** |
| 31 | **v2.1.6 Groove 集成 + WebKit 信号诚实化 + worklet 压缩** | `e073df5` **按帧寻址音符事件**（`noteAt`/`noteOnAt`/`noteOffAt`，绝对帧、按帧切块、队列 1024、`ready` 回报 128 帧固定延迟）**用户拍板发运**；新增**协议契约文档** `docs/notes/groove-host-protocol.md` + **防漂移门禁** `verify:worklet-protocol`（双向 13 条，已入 `verify` 链）；`grooveguard` 补输入守卫（缺 velocity 的 NaN、`atFrame:null` 当 0、非有限值整条忽略，合法路径逐位不变）。`workletmin` 把 AudioWorklet 资源构建期压缩（35.1 → 15.0 KB，**买回 19.7 KB**，dist 1555.2 → **1536.0**），并加「dist 资源就是生成的压缩副本 / <源码 60% / 单行」门禁与「用构建产物渲染音频」的单测。WebKit 侧：`pwa:105` 引擎离线重载具名跳过、`flow:407` 改断言受约束维度填充、`flow:246` 12 帧并集、`responsive` coarse 指针按读数跳过（只限 WebKit）、**`interact.dblclick` 改成真双击**、**§一.47 的 PWA 离线假红改成等启动遮罩自己的信号** | — | 中 | ✅ **v2.1.6** |
