...
test.describe('recorded takes', () => {
  ...
  test('merges with union or overwrite, and the two differ (P10.3)', async ({ page }) => {
    ...
    // keyboard spans MIDI 48-72, so the three extra notes stay inside it.) The
    // first pass deliberately avoids the imported bed's D4 (62): a note at the
    // same pitch within `TAKE_RESTRIKE` (0.05 s) *replaces* the bed note instead
    // of adding one, and whether the recorded start lands inside that window is
    // the wall-clock delay from pressing record to pressing the key — measured
    // at 0.033 s on WebKit and 0.12-0.36 s on Chromium, so the count below was
    // a coin flip on a 50 ms boundary rather than a statement about merging.
    await recordButton(page).click();
    await play(page, 63);
    await stopRecordButton(page).click();
    await expect(notes).toHaveCount(2);
