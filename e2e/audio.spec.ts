...
test.describe('audio settings', () => {
  ...
  test('preloads the DSP core before the start gesture', async ({ page }) => {
    ...
    //
    // Asked of the **server**, not read off the page's own responses: those
    // reported `""` on macOS Firefox while the probe showed the very same URL
    // answered `200 application/wasm`, because a reading taken through the page
    // cannot tell a real MIME regression from a cache artifact — an opaque
    // service-worker prefetch and a `304` revalidation both carry no
    // `Content-Type` (there is no body to describe). The request API goes to the
    // origin without the service worker or the page cache in between, so this
    // pins the property the browser actually depends on, and a regression here is
    // a server configuration change rather than a cache state.
    const served = await page.request.get(wasmRequests[0]);
    expect(served.status()).toBe(200);
    expect(served.headers()['content-type']).toContain('application/wasm');
    // And the engine reports that it took the streaming path: the audio
    // settings panel shows the core as `simd · streamed`. This is the end-to-end
    // half — a wrong `Content-Type` makes `instantiateStreaming` reject, and the
    // panel then says so instead.
    await page.locator('[data-act="settings"]').click();
    await page.getByRole('button', { name: '打开音频设置' }).click();
