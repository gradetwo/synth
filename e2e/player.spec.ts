...
test.describe('player', () => {
  ...
  test('lists the built-in songs and drives the transport', async ({ page }) => {
    ...
    //
    // The toggle is debounced against the *selection*: `PlayerPanel` ignores a
    // double click that arrives within 450 ms of a track changing
    // (`performance.now() - lastSelect.current < 450`), because the second click
    // of "double-click a track to select and play it" would otherwise pause what
    // the first click just started. Selecting the track above starts that window,
    // and these steps are far faster than a hand: measured on macOS WebKit, the
    // double click landed **86 ms** after the selection (271 ms with the
    // frame-free verbs) and was swallowed — `.player-play` never got `on`, 5 runs
    // out of 5, with native *and* frame-free clicks — while the same sequence
    // with an 818-970 ms gap resumed playback. So wait the window out: this
    // assertion is about the toggle, not about the debounce.
