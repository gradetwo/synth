 ...
 * minified worklet URL the build emits, `@/audio/engine`, the user-content
 * singletons and `@/i18n`) are replaced by empty stubs: the tools only use
 * `encodeWavBuffer`/`parseWav`, `params.ts`, `presets.ts` and `share.ts`, none
 * of which touch them.
 */
...
const STUBS = [
  ...
  // The worklet `?url` import is the build's minified asset now (the readable
  // source stays at `src/audio/worklet-processor.js`, which is read as text
  // below). esbuild has no `?url` loader, so without this stub the bundle fails
  // on a default export the asset does not have.
  '@/generated/worklet-processor.min.js?url',
  '@/audio/engine',
  '@/audio/userWave',
  '@/audio/ir',
  '@/audio/userSample',
